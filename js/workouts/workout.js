...
createApp({
  ...
  computed: {
    ...
    exerciseType() {
      if (!this.Rest) {
        return this.roundData[this.workout.round].Rep_Type_Linked_Exercises[
      ...
      if (!this.Rest)
        if (this.exerciseType == "Time") {
          return (this.workout.counter =
            parseInt(
              this.roundData[this.workout.round].Amounts_Name_Linked_Exercises[
                this.workout.exercises
              ]
            ) + 1);
        } else {
          return (this.workout.counter = parseInt(
            this.roundData[this.workout.round].Amounts_Name_Linked_Exercises[
              this.workout.exercises
            ]
          ));
        ...
        return (this.workout.counter =
          parseInt(
            this.roundData[this.workout.round].Amounts_Name_Linked_Exercises[1]
          ) + 1);
      }
    },
    amrapActive() {
      return this.roundData[this.workout.round].Amrap_Linked_Exercises[
        this.workout.exercises
      ];
    },
    amrapAmounts() {
      return this.roundData[this.workout.round]
        .Amrap_Exercise_Amount_Linked_Exercises;
    },
    exerciseVoice() {
      if (!this.Rest) {
        return this.roundData[this.workout.round].Audio_Source_Linked_Exercises[
          this.workout.exercises
        ].url;
      } else {
        return this.roundData[this.workout.round]
          .Audio_Source_Linked_Exercises[1].url;
      }
    },
    exerciseMax() {
      return this.exerciseData[this.workout.round][this.workout.exercises].Video
        .length;
    },
    exerciseMin() {
    ...
    exerciseMinData() {
      return isNaN(parseInt(this.workout.startDifficulty[this.exercise]) - 1)
        ? 0
        : this.min;
    },
    exerciseDiffs() {
    ...
    exerciseTitle() {
      return this.exerciseData[this.workout.round][this.workout.exercises]
        .Exercise_Category[0];
    },
    exerciseVideo() {
      return this.exerciseData[this.workout.round][this.workout.exercises]
        .Video[this.defaultDiffs].url;
    },
    exerciseIndex() {
  ...
  methods: {
    ...
    async intialRequest() {
      ...
      Wized.request.await("Load Round Info", (response) => {
        ...
        this.workout.startDifficulty =
          this.roundData[this.workout.round].Default_Diff_Level.split(", ");
        this.min = this.defaultDiffs;

        this.workout.roundAmount = roundRes.data.length;
        this.type =
          this.roundData[this.workout.round].Rep_Type_Linked_Exercises[
            this.workout.exercises
          ];
        this.isAmrap =
          this.roundData[this.workout.round].Amrap_Linked_Exercises[
            this.workout.exercises
          ];
      ...
      Wized.request.await("Load Diffculties Page 1", (response1) => {
        let page1Workouts = response1.data.filter(
          (workout) =>
            Array.isArray(workout.Workout_ID) &&
            workout.Workout_ID.includes(this.workout.id)
        );
        ...
        Wized.request.await("Load Diffculties Page 2", (response2) => {
          let page2Workouts = response2.data.filter(
            (workout) =>
              Array.isArray(workout.Workout_ID) &&
              workout.Workout_ID.includes(this.workout.id)
          );
    ...
    amrapRest() {
      this.roundData[
        this.workout.round
      ].Exercise_Title_Linked_Exercises.forEach((value, index) => {
    ...
    ChangeExercise(play, video, voice, input, siren) {
      ...
      if (input == 1) {
        ...
        voice.src =
          this.roundData[this.workout.round].Audio_Source_Linked_Exercises[
            this.workout.exercises
          ].url;
        ...
        // Change Exercises Number
        this.workout.exercises =
          this.exerciseData[this.workout.round].length - 1;
        this.popup = false;
      }
    ...
    PrevExercise() {
      ...
      // Round Start
        ...
        // Exercise Change
        this.min =
          parseInt(this.workout.startDifficulty[this.workout.exercises]) - 1;
        this.popup = false;
        clearInterval(timer);
        this.ChangeExercise(
          this.$refs.play,
    ...
    NextExercise(siren, amrap) {
      ...
      if (
        this.workout.exercises ==
          this.exerciseData[this.workout.round].length - 1 &&
        this.workout.round + 1 !== this.roundData.length &&
        this.amrapActive !== "True"
      ) {
        this.roundSkipped = false;
        this.popup = true;
        clearInterval(timer);
      ...
      // Finished Change
        ...
        this.workout.exercises + 1 ==
          this.exerciseData[this.workout.round].length &&
        this.workout.round + 1 == this.roundData.length
      ) {
        this.popup = true;
        this.completed = true;
    ...
    ChangeDifficulty(input, video) {
      ...
      if (input == 0) {
        ...
        video.src =
          this.exerciseData[this.workout.round][this.workout.exercises].Video[
            this.min
          ].url;
        setTimeout(() => {
          this.loadedExercise = false;
    ...
    ChangeAmrapDifficulty(input, video, index, number) {
      ...
      if (input == 0) {
        this.amrapData.currentIndices[number] =
          this.amrapData.currentIndices[number] - 1;
        const newDiff = this.amrapData.currentIndices[number];
        index[number].innerHTML = newDiff;

        video.src =
          this.exerciseData[this.workout.round][srcIndex].Video[newDiff].url;
        setTimeout(() => {
          this.loadedExercise = false;
          // Video Condtion Play/Pause
          if (video.paused) {
        ...
        video.src =
          this.exerciseData[this.workout.round][srcIndex].Video[
            this.amrapData.diff[srcIndex][newDiff]
          ].url;
        setTimeout(() => {
          this.loadedExercise = false;
          // Video Condtion Play/Pause
    ...
    AmrapVideo(video) {
      ...
      if (this.amrapActive == "True" && this.amrapPlayed !== true) {
        ...
        video.src =
          this.exerciseData[this.workout.round][srcIndex].Video[
            this.amrapData.diff[srcIndex][index]
          ].url;
  ...
  mounted() {
    ...
    this.LoadedAudioEnabled(
      this.$refs.sirenText,
      this.$refs.sirenToggle,
      this.$refs.voiceText,
      this.$refs.voiceToggle
    );
